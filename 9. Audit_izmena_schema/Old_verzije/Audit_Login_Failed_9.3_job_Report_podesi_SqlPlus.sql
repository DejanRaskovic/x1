...
Dejan Raskovic', 
		...
		@owner_login_name=N'DELTABANK\admin_vmarjan', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
where slanje = 0 and LoginName not in ('alfa155','DELTABANK\quser')
--where slanje = 0
...
', 
		...
		@output_file_name=N'C:\SqlPlus\Logs\_LoginFailedReport.txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
